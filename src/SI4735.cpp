...
si4735_eeprom_patch_header SI4735::downloadPatchFromEeprom(int eeprom_i2c_address)
{
    ...
    if (eep.refined.patch_id[0] != 'p') 
    {
        strcpy((char *)eep.refined.patch_id, "patch NAME error");
        return eep;
    }
    if ((int)eep.refined.patch_size != 8840) {
        strcpy((char *)eep.refined.patch_id, "patch SIZE error");
        return eep;
    }
    ...
    for (i = 0; i < (int)eep.refined.patch_size; i += 8)
    {
        ...
        if (cmd_status != 0x80)
        {
            strcpy((char *)eep.refined.patch_id, "transfer error!");
            return eep;
        }
        offset += 8; // Start processing the next 8 bytes
    }
    strcpy((char *)eep.refined.patch_id, "success");
    delay(50);
    return eep;
}
